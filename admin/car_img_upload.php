    ...
    if (!empty($_FILES["image"]["name"]) && !empty($_FILES["sideimage1"]["name"]) && !empty($_FILES["sideimage2"]["name"]) && !empty($_FILES["sideimage3"]["name"])) {
        ...
        if (in_array($fileType0, $allowTypes) && in_array($fileType1, $allowTypes) && in_array($fileType2, $allowTypes) && in_array($fileType3, $allowTypes)) {
            ...
            $arahan_sql_simpan = "
                ...
                VALUES (:image, :sideimages1, :sideimages2, :sideimages3, SYSDATE)
            ";
            ...
            oci_bind_by_name($laksana_arahan, ':image', $imgContent0);
            oci_bind_by_name($laksana_arahan, ':sideimages1', $imgContent1);
            oci_bind_by_name($laksana_arahan, ':sideimages2', $imgContent2);
            oci_bind_by_name($laksana_arahan, ':sideimages3', $imgContent3);
